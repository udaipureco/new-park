...
document.addEventListener("DOMContentLoaded", function () {
  try {
    ...
    if (d.items && d.items.ev > 0) {
      evHTML += infoRow("EV Cart Ride (" + d.items.ev + ")", "\u20B9" + (d.items.ev * 20));
    }
    if (evEl) evEl.innerHTML = evHTML || '<div class="ticket-info-row"><span>None</span></div>';
