...
document.addEventListener("DOMContentLoaded", function () {
  try {
    ...
    if (d.type !== "ENTRY") { window.location.replace("booking.html"); return; }
    ...
    var html = "";
    if (d.items) {
      if (d.items.adult > 0) html += infoRow("Adult (" + d.items.adult + ")", d.items.adult * 10);
      if (d.items.child > 0) html += infoRow("Child (" + d.items.child + ")", d.items.child * 5);
    }
    if (entryEl) entryEl.innerHTML = html || '<p class="no-items">None selected</p>';
